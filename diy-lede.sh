...
# 修改openwrt登陆地址,把下面的192.168.22.2修改成你想要的就可以了
sed -i 's/192.168.1.1/192.168.22.2/g' ./package/base-files/files/bin/config_generate


# 修改主机名字，把Cool-Lede修改你喜欢的就行（不能纯数字或者使用中文）
sed -i 's/OpenWrt/Cool-Lede/g' ./package/base-files/files/bin/config_generate
